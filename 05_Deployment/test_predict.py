...
#### Predict with FastAPI
# Adding a new record to customers in dict
response = requests.post("http://localhost:8000/", json=customer).json()
response = requests.post("http://localhost:8000/", json=customer2).json()

# Querying customer that we just added
response = requests.get("http://localhost:8000/customer/XXXG-00W0").json()
print(response)

response = requests.get("http://localhost:8000/customer/GN-001").json()
print(response)

## Querying customer not exist in customer pydantic dict will return not found
response = requests.get("http://localhost:8000/customer/XXXG-002").json()
print(response)

# Updating record on a customer
print(requests.put("http://localhost:8000/update/GN-001?paymentmethod=credit_card").json())
