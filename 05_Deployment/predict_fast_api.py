...
@app.get("/customers/")
def query_customer_by_parameters(
    contract: Optional[str] = None,
    paperlessbilling: Optional[str] = None,
    internetservice: Optional[str] = None,
) -> Dict[str, Union[Selection, List[Customer]]]:
    def check_item(item: Customer):
        """Check if the item matches the query arguments from the outer scope."""
        return all(
            (
                contract is None or item.contract == contract,
                paperlessbilling is None or item.paperlessbilling == paperlessbilling,
                internetservice is None or item.internetservice == internetservice
            )
        )
